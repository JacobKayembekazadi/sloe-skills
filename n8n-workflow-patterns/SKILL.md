...
version: 1.0
description: Proven architectural patterns for building n8n workflows — webhook processing, HTTP API integration, database operations, AI agent workflows, error handling, and scheduled tasks. Built from real automation workflows in production.
updated: 2026-03-03
---
...
**2026-03-03 — Job hunt automation (prospect research + outreach pipeline):**
- Webhook → n8n → HubSpot sync: always add a 120ms sleep between HubSpot API calls — rate limit is 100 req/10s and batch operations hit it silently
- HTTP API integration pattern: `jq` not available on all VPS environments — use Python `json.tool` or Node.js for JSON parsing in Code nodes
- Error handling: n8n's built-in error workflow fires but doesn't surface to the user — always add a final Telegram/Slack notify node on the error branch
- AI agent workflows: Gemini/OpenAI node timeouts at 30s by default — for long-running AI operations, use the webhook response pattern (accept → process async → callback)

**2026-03-03 — Ion Houston demo pipeline (5 client demos, live data):**
- Supabase HTTP integration: use the REST API with service_role key for server-side n8n workflows, never anon key
- Report generation workflow: Resend email delivery from n8n is faster than Zapier equivalent — use `application/json` content-type with the Resend HTTP node
- Scheduled workflows: n8n cron doesn't handle timezone drift well — always anchor to UTC and convert in the workflow, not in the cron expression
